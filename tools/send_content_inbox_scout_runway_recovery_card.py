...
async def _run(plan_id: str) -> dict[str, object]:
    ...
    plan = story_pack_control.current_frontal_reference_retry_plan(
        main.NAZ_STORY_PACK_ROOT, plan_id
    )
    text = story_pack_control.current_frontal_reference_retry_card(
        main.NAZ_STORY_PACK_ROOT, plan_id
    )
    ...
    async with Bot(token=main.BOT_TOKEN) as bot:
        message = await bot.send_message(
            ...
            text=text,
            reply_markup=main.inbox_scout_runway_recovery_keyboard(plan_id),
        )
    return {
        "status": "awaiting_current_frontal_retry_approval",
        **plan,
